...
{/* TODO: mention other session types here*/}

Record server and Kubernetes sessions are available for listing and replay for
authorized users via `tsh`.  The same listings, including Windows desktop
sessions, are available in the Web Console under Session Recordings in the
Activity section.

### List recordings

Get the list of recorded sessions. By default, recordings for the past 24 hours
are shown.  The time range can be customized with the `--last`, `--from-utc`,
and `--to-utc` flags.
...
{/* TODO: Talk about differen resource types*/}
{/* TODo: Talk about playing files*/}
