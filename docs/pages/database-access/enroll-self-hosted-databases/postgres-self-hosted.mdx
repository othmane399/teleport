...
The Teleport Database Service proxies user connections by connecting to the
database using mutual TLS. You can configure your database to trust either the
Teleport certificate authority for databases or your own certificate authority.
If you choose the latter, Teleport still manages user authentication and
...
- A certificate authority if you plan to use one for mTLS between your database
  and the Teleport Database Service. Otherwise, you can use the Teleport
  database CA.
...
If you plan to use the Teleport database CA, run the following command to export
Teleport's certificate authority and a generate certificate/key pair (the `ttl`
flag sets the validity of the certificate to one year):
...
The command will create 3 files: 

|File|Description|
|---|---|
|`server.cas`| x.509 CA certificate for verifying the Teleport Database Service certificate |
|`server.crt`| x.509 certificate that the database can use to authenticate to the Teleport Database Service. Must have the address of the database as a subject.|
|`server.key`| Private key for the database. |

If you are using your own certificate authority, use it to create these three
files with the file names given above.
